...
class AlignedNpzBaselineDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        A_tensor = self.read_npz_to_tensor(A_path)
        B_tensor = self.read_npz_to_tensor(B_path)

        return {'A': A_tensor, 'B': B_tensor, 'A_paths': A_path, 'B_paths': B_path}

    def read_npz_to_tensor(self, npz_path):
        try:
